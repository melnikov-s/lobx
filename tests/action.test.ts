...
describe("action tests", () => {
	it("immediately executes the function passed in", () => {
		let count = 0;
		runInAction(() => count++);
		expect(count).toBe(1);
	});

	it("updates observable values", () => {
		const o = observable.box(0);
		runInAction(() => o.set(1));
		expect(o.get()).toBe(1);
	});

	it("runInAction returns the result of the action", () => {
		expect(runInAction(() => 1)).toBe(1);
	});

	it("only calls reactions when the action is completed", () => {
	...
	it("computed values are updated within an action", () => {
		let countC = 0;
		let countA = 0;
		const o1 = observable.box(0);
		const o2 = observable.box(0);
	...
	it("does not trigger a change when an observable did not end up producing a new value", () => {
		let count = 0;
		const o = observable.box(0);
	...
	it("can create an action to execute at any time", () => {
		const o1 = observable.box(0);
		const o2 = observable.box(0);
		let result = 0;
		let count = 0;
	...
	it("created action returns the result of the action", () => {
		const myAction = action(() => 1);
		expect(myAction()).toBe(1);
	});

	it("can execute an action within an action", () => {
		let countC = 0;
		let countA = 0;
		const o1 = observable.box(0);
		const o2 = observable.box(0);
	...
	//from mobx
	it("action in autorun does keep / make computed values alive", () => {
		let calls = 0;
