...
test("map.clear should not be tracked", () => {
...
test("map keys should be coerced to strings correctly", () => {
	const m = map();
	m.set(1, true);
	m.delete(1);
	expect(keys(m)).toEqual([]);
...
test("iterators should be resilient to concurrent delete operation", () => {
